...
ooze_dock_launch_about (MetaContext *context)
{
  ...
  if (!context)
    {
      g_warning ("OozeDock: no compositor context for launching ooze-about");
      return;
    }

  if (!cmd)
    {
      g_warning ("OozeDock: ooze-about not found in PATH");
      return;
    }
  ...
  if (!meta_wayland_client_new_subprocess (context, launcher, argv, &error))
    g_warning ("OozeDock: failed to launch ooze-about: %s", error->message);
  else
    g_print ("OozeDock: launched ooze-about\n");
}
...
void
ooze_dock_populate_container (MetaContext   *context G_GNUC_UNUSED,
                            MetaDisplay   *display,
                            ClutterActor  *stage,
                            ClutterActor  *container)
{
...

  {
    ...
    spot_launcher = ooze_dock_create_spot_launcher (stage, display);
    ...
    command_launcher = ooze_dock_create_command_launcher (stage, display);
    ...
    ear_launcher = ooze_dock_create_ear_launcher (stage, display);
    ...
    king_launcher = ooze_dock_create_king_launcher (stage, display);
    ...
    pak_launcher = ooze_dock_create_pak_launcher (stage, display);
    ...
    trash_launcher = ooze_dock_create_trash_launcher (stage, display);
  ...
  g_timeout_add (600, ooze_dock_update_indicators_cb, indicator_ctx);
}
