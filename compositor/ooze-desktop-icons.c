...
static const OozeDesktopIconDef desktop_icon_defs[] = {
  { "Linux HD", "/", linux_hd_icon_names, 0.85f, 0.22f, 0.18f },
  { NULL, NULL, home_icon_names, 0.22f, 0.48f, 0.92f },
};

static void
ooze_desktop_icon_data_free (gpointer data)
{
  OozeDesktopIconData *icon_data = data;
...
static void
ooze_desktop_icon_launch (OozeDesktopIconData *icon_data,
                        const char        *path)
{
  ...
  ooze_dock_launch_spot_path (icon_data->context, path);
}

static gboolean
ooze_desktop_icon_pressed (ClutterActor *actor,
                         ClutterEvent *event,
                         OozeDesktopIconData *icon_data)
{
  const char *path;
  guint32 time;
  guint interval;
  ...
  if (icon_data->last_click_path &&
    ...
    {
      ooze_desktop_icon_launch (icon_data, path);
                        ...
                        gfloat              b)
{
  ...
  if (icon_names)
    icon_content = ooze_dock_themed_icon_content (ref_actor,
                                                display,
                                                icon_names,
                                                (int) DESKTOP_ICON_SIZE);
  if (!icon_content)
    icon_content = ooze_aqua_dock_icon_content (ref_actor,
                                              (int) DESKTOP_ICON_SIZE,
                                              r,
                                              g,
                                              b);
  if (icon_content)
    {
      ...
      ooze_aqua_actor_set_scaled_content (image,
  ...
  label_content = ooze_aqua_text_content (ref_actor,
                                        OOZE_UI_FONT,
                                        label,
                                        1.0f,
                                        1.0f,
  ...
  if (label_content)
    ooze_aqua_actor_set_content (label_actor,
  ...
  g_signal_connect (icon,
                    "button-press-event",
                    G_CALLBACK (ooze_desktop_icon_pressed),
                    icon_data);
  ...
  for (i = 0; i < G_N_ELEMENTS (desktop_icon_defs); i++)
    {
      ...
      icon = ooze_desktop_icon_create (ref_actor,
                                     display,
                                     icon_data,
                                     label,
                                     path,
