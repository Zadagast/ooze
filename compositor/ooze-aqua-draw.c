...
ooze_aqua_get_context (ClutterActor *ref_actor)
{
  while (ref_actor)
    {
...
static CoglTexture *
ooze_aqua_surface_to_texture (ClutterActor     *ref_actor,
                            cairo_surface_t  *surface)
{
  ...
  clutter_context = ooze_aqua_get_context (ref_actor);
  if (!clutter_context)
    return NULL;
  ...
  texture = cogl_texture_2d_new_from_data (cogl_context,
                                           ...
                                           &error);
  if (!texture)
    g_warning ("Ooze: failed to upload texture: %s",
...
ClutterContent *
ooze_aqua_content_from_surface (ClutterActor    *ref_actor,
                              cairo_surface_t *surface)
{
  g_autoptr (CoglTexture) texture = NULL;

  texture = ooze_aqua_surface_to_texture (ref_actor, surface);
  if (!texture)
    return NULL;
...
static char *
ooze_aqua_find_data_path (const char *filename)
{
  const char *env_dir;
...
static GdkPixbuf *
ooze_aqua_load_data_pixbuf (const char *filename,
                          int         width,
                          int         height)
{
  g_autofree char *path = ooze_aqua_find_data_path (filename);
  g_autoptr (GError) error = NULL;
                             ...
                             GdkPixbuf    *pixbuf)
{
  ...
  if (!texture)
    {
      g_warning ("Ooze: failed to upload pixbuf texture: %s",
                 error ? error->message : "unknown error");
      return NULL;
    }
...
static void
ooze_aqua_draw_feather (cairo_t             *cr,
                      gdouble              width,
                      gdouble              height,
...
static void
ooze_aqua_rounded_rect (cairo_t *cr,
                      gdouble  x,
                      gdouble  y,
                      gdouble  w,
                      gdouble  h,
...
static void
ooze_aqua_draw_pinstripe (cairo_t             *cr,
                        int                  width,
                        int                  height,
                        const OozeAquaPalette *palette)
{
...
ClutterContent *
ooze_aqua_pinstripe_content (ClutterActor *ref_actor,
                           int           width,
                           int           height)
{
  cairo_surface_t *surface;
  ...
  ooze_aqua_draw_pinstripe (cr, width, height, ooze_theme_get_palette (NULL));
  cairo_destroy (cr);
...
ClutterContent *
ooze_aqua_wallpaper_content (ClutterActor *ref_actor,
                           int           width,
                           int           height)
{
  cairo_surface_t *surface;
...
ClutterContent *
ooze_aqua_dock_plate_content (ClutterActor *ref_actor,
                            int           width,
                            int           height)
{
  cairo_surface_t *surface;
  ...
  gdouble radius;
  const OozeAquaPalette *palette = ooze_theme_get_palette (NULL);
  gdouble glass_r = ooze_theme_is_dark (NULL) ? 0.14 : 1.0;
  gdouble glass_g = ooze_theme_is_dark (NULL) ? 0.14 : 1.0;
  gdouble glass_b = ooze_theme_is_dark (NULL) ? 0.16 : 1.0;
  ...
  ooze_aqua_rounded_rect (cr, 2.0, 2.0, width - 4.0, (height - 4.0) * 0.45, radius - 2.0);
  cairo_fill (cr);
...
ClutterContent *
ooze_aqua_dock_icon_content (ClutterActor *ref_actor,
                           int           size,
                           gfloat        r,
                           gfloat        g,
                           gfloat        b)
{
  ...
  ooze_aqua_rounded_rect (cr, 0.5, 0.5, size - 1.0, size - 1.0, radius);
  ...
  ooze_aqua_rounded_rect (cr, 1.0, 1.0, size - 2.0, size * 0.52, radius - 1.0);
  cairo_fill (cr);
  cairo_pattern_destroy (gloss);

  cairo_set_source_rgba (cr, 1.0, 1.0, 1.0, 0.18);
  ooze_aqua_rounded_rect (cr, 2.0, 2.0, size - 4.0, size * 0.22, radius - 2.0);
  cairo_fill (cr);
...
ClutterContent *
ooze_aqua_dock_reflection_content (ClutterActor   *ref_actor,
                                 ...
                                 int             reflect_h)
{
  ...
  content = ooze_aqua_content_from_surface (ref_actor, dst_surface);
...
ClutterContent *
ooze_aqua_traffic_light_content (ClutterActor *ref_actor,
                               int           size,
                               gfloat        r,
                               gfloat        g,
                               gfloat        b)
...
ClutterContent *
ooze_aqua_ooze_button_content (ClutterActor *ref_actor,
                             int          *width_out,
                             int          *height_out)
{
  cairo_surface_t *surface;
  ...
  ooze_aqua_rounded_rect (cr, 1.5, 1.5, width - 3.0, (height - 3.0) * 0.28, radius - 1.0);
...
ClutterContent *
ooze_aqua_menu_feather_content (ClutterActor *ref_actor,
                              int           width,
                              int           height)
{
  cairo_surface_t *surface;
  ...
  surface = cairo_image_surface_create (CAIRO_FORMAT_ARGB32, width, height);
  cr = cairo_create (surface);
  ooze_aqua_draw_feather (cr, width, height, ooze_theme_get_palette (NULL));
  cairo_destroy (cr);
  content = ooze_aqua_content_from_surface (ref_actor, surface);
  cairo_surface_destroy (surface);
...
ClutterContent *
ooze_aqua_spot_icon_content (ClutterActor *ref_actor,
                           MetaDisplay  *display,
                           int           logical_size)
{
...
ClutterContent *
ooze_aqua_apple_logo_content (ClutterActor *ref_actor,
                            int           size)
{
  return ooze_aqua_menu_feather_content (ref_actor, size, size);
}

ClutterContent *
ooze_aqua_text_content_ellipsize (ClutterActor *ref_actor,
                                ...
                                int          *height_out)
{
  ...
  if (max_width_px > 0)
    width = MIN (width, max_width_px + 4);
...
ClutterContent *
ooze_aqua_text_content (ClutterActor *ref_actor,
                      const char   *font_desc,
                      const char   *text,
                      gfloat        r,
                      gfloat        g,
                      ...
                      int          *height_out)
{
  return ooze_aqua_text_content_ellipsize (ref_actor, font_desc, text,
                                         r, g, b, 0, width_out, height_out);
}

void
ooze_aqua_actor_set_content (ClutterActor   *actor,
                           ClutterContent *content,
                           int             width,
                           int             height)
{
...
void
ooze_aqua_actor_set_scaled_content (ClutterActor   *actor,
                                  ClutterContent *content,
                                  int             display_w,
                                  int             display_h,
                                  int             texture_w,
...
int
ooze_aqua_icon_texture_size (MetaDisplay *display,
                           int          logical_size)
{
  gfloat scale = 2.0f;
