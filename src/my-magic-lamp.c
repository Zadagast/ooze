...
typedef enum {
  MY_LAMP_SIDE_BOTTOM = 0,
  MY_LAMP_SIDE_TOP,
  MY_LAMP_SIDE_LEFT,
  MY_LAMP_SIDE_RIGHT,
} MyLampSide;

G_DECLARE_FINAL_TYPE (MyMagicLampEffect,
                      my_magic_lamp_effect,
                      MY,
                      MAGIC_LAMP_EFFECT,
                      ClutterDeformEffect)

struct _MyMagicLampEffect
{
...
G_DEFINE_TYPE (MyMagicLampEffect, my_magic_lamp_effect, CLUTTER_TYPE_DEFORM_EFFECT)

static void
my_magic_lamp_effect_finish (MyMagicLampEffect *self,
                             gboolean           notify)
{
  ClutterActor *actor;
  ...
  if (self->timeline)
    {
      if (self->new_frame_id)
        {
          g_signal_handler_disconnect (self->timeline, self->new_frame_id);
          self->new_frame_id = 0;
        }
      if (self->completed_id)
        {
          g_signal_handler_disconnect (self->timeline, self->completed_id);
          self->completed_id = 0;
        }
  ...
  if (notify && done && window_actor && !meta_window_actor_is_destroyed (window_actor))
...
static void
my_magic_lamp_effect_on_completed (ClutterTimeline   *timeline G_GNUC_UNUSED,
                                   MyMagicLampEffect *self)
{
  my_magic_lamp_effect_finish (self, TRUE);
}

static void
my_magic_lamp_effect_on_new_frame (ClutterTimeline   *timeline,
...
static void
my_magic_lamp_effect_init_geometry (MyMagicLampEffect *self,
                                    ClutterActor      *actor)
{
  ...
  if (self->icon_y + self->icon_h >= self->monitor_h - MAGIC_LAMP_EPSILON)
    ...
    {
      self->icon_side = MY_LAMP_SIDE_LEFT;
      self->icon_x = 0.f;
      self->icon_w = 0.f;
    }
  else if (self->icon_x + self->icon_w >= self->monitor_w - MAGIC_LAMP_EPSILON)
    {
      self->icon_side = MY_LAMP_SIDE_RIGHT;
...
static void
my_magic_lamp_effect_set_actor (ClutterActorMeta *meta,
                                ClutterActor     *actor)
{
  MyMagicLampEffect *self = MY_MAGIC_LAMP_EFFECT (meta);
  guint duration;
  gdouble area_factor;

  CLUTTER_ACTOR_META_CLASS (my_magic_lamp_effect_parent_class)->set_actor (meta, actor);

  if (!actor || self->initialized)
    return;

  self->initialized = TRUE;
  my_magic_lamp_effect_init_geometry (self, actor);
  ...
  self->new_frame_id =
    g_signal_connect (self->timeline, "new-frame",
                      G_CALLBACK (my_magic_lamp_effect_on_new_frame), self);
  self->completed_id =
    g_signal_connect (self->timeline, "completed",
                      G_CALLBACK (my_magic_lamp_effect_on_completed), self);
  clutter_timeline_start (self->timeline);
}

static gboolean
my_magic_lamp_effect_modify_paint_volume (ClutterEffect      *effect G_GNUC_UNUSED,
...
static void
my_magic_lamp_effect_deform_vertex (ClutterDeformEffect  *effect,
                                    gfloat                width,
                                    gfloat                height,
                                    ClutterTextureVertex *v)
{
  MyMagicLampEffect *self = MY_MAGIC_LAMP_EFFECT (effect);
...
static void
my_magic_lamp_effect_dispose (GObject *object)
{
  MyMagicLampEffect *self = MY_MAGIC_LAMP_EFFECT (object);

  my_magic_lamp_effect_finish (self, FALSE);

  G_OBJECT_CLASS (my_magic_lamp_effect_parent_class)->dispose (object);
}

static void
my_magic_lamp_effect_class_init (MyMagicLampEffectClass *klass)
{
  ...
  object_class->dispose = my_magic_lamp_effect_dispose;
  meta_class->set_actor = my_magic_lamp_effect_set_actor;
  effect_class->modify_paint_volume = my_magic_lamp_effect_modify_paint_volume;
  deform_class->deform_vertex = my_magic_lamp_effect_deform_vertex;
}

static void
my_magic_lamp_effect_init (MyMagicLampEffect *self)
{
...
static MyMagicLampEffect *
my_magic_lamp_effect_new (MetaPlugin         *plugin,
                          gboolean            unminimize,
                          gfloat              icon_x,
                          gfloat              icon_y,
                          gfloat              icon_w,
                          ...
                          gpointer            user_data)
{
  ...
  self = g_object_new (my_magic_lamp_effect_get_type (), NULL);
  self->plugin = plugin;
...
void
my_magic_lamp_cancel (MetaWindowActor *actor)
{
  ...
  if (effect)
    my_magic_lamp_effect_finish (MY_MAGIC_LAMP_EFFECT (effect), FALSE);
}

void
my_magic_lamp_run (MetaPlugin         *plugin,
                   ...
                   gpointer            user_data)
{
  ...
  my_magic_lamp_cancel (actor);

  if (unminimize)
    clutter_actor_show (CLUTTER_ACTOR (actor));

  effect = my_magic_lamp_effect_new (plugin, unminimize,
