...
static gboolean
my_icons_has_elementary_theme (const char *data_root)
{
...
static char *
my_icons_get_data_root_from_exe (void)
{
#ifdef __linux__
  char exe_path[4096];
  ssize_t len;
  ...
  if (len > 0)
    {
      ...
      if (my_icons_has_elementary_theme (data))
        {
...
my_icons_get_data_root (void)
{
  ...
  {
    g_autofree char *from_exe = my_icons_get_data_root_from_exe ();
...
#ifdef OOZE_DATA_DIR
  if (my_icons_has_elementary_theme (OOZE_DATA_DIR))
...

  {
    ...
    if (my_icons_has_elementary_theme (cwd_data))
...
char *
my_icons_get_icons_dir (void)
{
  g_autofree char *data_root = my_icons_get_data_root ();

  return g_build_filename (data_root, "icons", NULL);
}

char *
my_icons_get_elementary_dir (void)
{
...
static gboolean
my_icons_path_in_search_list (const char *list,
                              const char *path)
{
  const char *cursor;
  gsize path_len;
...
void
my_icons_setup_environment (void)
{
  g_autofree char *data_root = my_icons_get_data_root ();
  const char *existing;
  ...
  if (!my_icons_path_in_search_list (existing, data_root))
    {
...
static gboolean
my_icons_has_index_theme (const char *dir)
{
  g_autofree char *index = NULL;
...
gboolean
my_icons_theme_is_available (void)
{
  g_autofree char *local = my_icons_get_elementary_dir ();

  if (my_icons_has_index_theme (local))
    return TRUE;

  return my_icons_has_index_theme ("/usr/share/icons/" OOZE_ICON_THEME);
}
...
my_icons_apply (void)
{
  ...
  if (!my_icons_theme_is_available ())
    g_warning ("Ooze: elementary icon theme not found; run ninja -C build to fetch icons");
}

void
my_icons_apply_to_launcher (GSubprocessLauncher *launcher)
{
  g_autofree char *data_root = my_icons_get_data_root ();
